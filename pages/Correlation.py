...
st.title("Commercial CSI - Correlation Analysis")

# Step 1: Select Brand
brand = st.selectbox("Select Brand", ["TATA", "MB", "Agri"])

# Define dataset paths for each brand
brand_paths = {
    "TATA": "TATA_Corr.csv",
    "MB": "MB_Corr.csv",
    "Agri": "MnM_Corr.csv"
...
# Load dataset based on selected brand
csv_path = brand_paths[brand]
df = pd.read_csv(csv_path)
df.columns = df.columns.str.strip()
df_cleaned = df.dropna()

# Step 2: Select Branch (only if TATA is selected)
if brand == "TATA":
    # Check if the 'PLANT' column exists for TATA dataset
    if "PLANT" in df_cleaned.columns:
        ...
        st.warning("'PLANT' column not found in TATA dataset. Using the first available branch.")
        branch_df = df_cleaned  # Use entire dataset for TATA if 'PLANT' is missing
else:
    # For Benz and Mahindra, we do not have the 'PLANT' column, use the whole dataset
    branch_df = df_cleaned
...
if selected_columns and not branch_df.empty:
    ...
    # ✅ Overall Branch Correlation (only for TATA)
    if brand == "TATA":
