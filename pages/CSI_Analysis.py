...
# Select the brand from a dropdown
brand = st.selectbox("Select Brand", ["TATA", "MB", "Agri"])

# Define the file paths for datasets and models based on brand
brand_paths = {
    "TATA": {"data": "TATA_Model.csv", "model": "TATA_ElasticNet.joblib"},
    "MB": {"data": "MB_Model.csv", "model": "MB_ElasticNet.joblib"},
    "Agri": {"data": "MnM_Model.csv", "model": "MnM_Model.joblib"}
}

# Load the corresponding dataset and model based on the brand
df = pd.read_csv(brand_paths[brand]["data"])
model = joblib.load(brand_paths[brand]["model"])
...
st.markdown(
    f"""
    <h2 style='text-align: center;'>{brand} CSI 2024/2025</h2>
    """,
    unsafe_allow_html=True
)
